...
<section style="padding:2rem 0;">
    ...
    <div class="container" style="max-width:700px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;flex-wrap:wrap;gap:1rem;">
            <a href="<?= base_url('/client/commande/'.$commande['id']) ?>" style="color:var(--gray);">← Retour à la commande</a>
            <button onclick="window.print()" class="btn btn-dark">🖨️ Imprimer</button>
        </div>
            ...
            <div class="facture-body">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:2rem;margin-bottom:1.5rem;">
                    <div>
                        ...
                        <?php if ($facture['telephone']): ?><span style="color:var(--gray);font-size:.85rem;"><?= esc($facture['telephone']) ?></span><?php endif; ?>
                    </div>
                    <div>
                ...
                <table class="table" style="margin-bottom:1.5rem;">
                    <thead><tr><th>Description</th><th>Prix unit.</th><th>Qté</th><th>Total</th></tr></thead>
                    <tbody>
                ...
                <div style="max-width:280px;margin-left:auto;">
                    <div style="display:flex;justify-content:space-between;padding:.4rem 0;font-size:.9rem;border-bottom:1px solid var(--light-2);">
                        <span style="color:var(--gray);">Sous-total HT</span><span><?= number_format($facture['montant_ht'], 0, ',', '.') ?> FCFA</span>
                    </div>
                    <div style="display:flex;justify-content:space-between;padding:.4rem 0;font-size:.9rem;border-bottom:1px solid var(--light-2);">
                        <span style="color:var(--gray);">TVA (0%)</span><span>0 FCFA</span>
                    </div>
                    <div class="facture-total" style="margin-top:.7rem;">
                        <span>Total TTC</span><span><?= number_format($facture['montant_ttc'], 0, ',', '.') ?> FCFA</span>
                    </div>
                </div>
...
<style>@media print { nav,.footer,.btn { display:none !important; } body { background:#fff; } }</style>
